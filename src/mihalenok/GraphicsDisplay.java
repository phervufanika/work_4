...
public class GraphicsDisplay extends JPanel {
    ...
    private boolean ShowCriticalValues = false;

    public void ShowCriticalValues(boolean ShowCriticalValues) {
    ...
    public GraphicsDisplay() {
        ...
        BasicStroke dashed = new BasicStroke(3, BasicStroke.CAP_BUTT,
                ...
                0, new float[]{16, 4, 16, 4, 16, 4, 4, 4, 4, 4, 4}, 0);
        ...
        for (Double[] point: graphicsData) {
            ...
            if (point[0] >= viewport[0][0] && point[1] <= viewport[0][1] && point[0] <= viewport[1][0]) {
                ...
                if (i_n == this.selectedMarker) {
                    ...
                    canvas.draw(new Ellipse2D.Double(center.getX() - 5.5, center.getY() - 5.5, 11, 11));
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minY<=0.0 && maxY>=0.0) {
            ...
            if(ShowCriticalValues) {
                ...
                Point2D MinXSpot = xyToPoint(minX, 0.025);
                Point2D MaxXSpot = xyToPoint(maxX - 1, 0.1);
                Point2D MinYSpot = xyToPoint(0.025, minY + 0.06);
                Point2D MaxYSpot = xyToPoint(0.06, maxY - 0.210);

                canvas.drawString(String.valueOf(minX), (int) MinXSpot.getX(), (int) MinXSpot.getY());
                canvas.drawString(String.valueOf(maxX), (int) MaxXSpot.getX(), (int) MaxXSpot.getY());
                canvas.drawString(String.valueOf(minY), (int) MinYSpot.getX(), (int) MinYSpot.getY());
                canvas.drawString(String.valueOf(maxY), (int) MaxYSpot.getX(), (int) MaxYSpot.getY());
            }
        }
    }
